 ...
 * @version     $Id: class.CeonURIMappingInstallationCheck.php 1054 2012-09-22 15:45:15Z conor $
 */
...
class CeonURIMappingInstallationCheck extends CeonURIMappingVersion
{
	 ...
	 * @access  protected
	 */
	var $_config_files_error_messages = array();
	 ...
	 * @access  protected
	 */
	var $_modified_core_files_error_messages = array();
	 ...
	 * @access  protected
	 */
	var $_old_files_dirs_error_messages = array();
	 ...
	 */
	function _lookUpAutomaticVersionCheckingEnabled()
	{
		global $db;
	 ...
	 * @return  none
	 */
	function _checkConfigureFilesSettings()
	{
	 ...
	 * @return  none
	 */
	function _checkModifiedCoreFiles()
	{
		...
		foreach ($core_file_modifications as $core_file_modification) {
			...
			for ($i = 0, $n = sizeof($core_file_modification['new_snippets']); $i < $n; $i++) {
			...
			for ($i = 0, $n = sizeof($core_file_modification['old_snippets']); $i < $n; $i++) {
				...
				if ($num_new_code_snippets_present == 0) {
					...
					if ($num_missing_snippets == 1) {
						...
						if ($core_file_modification['num_mods'] == 2) {
							...
							if (sizeof($new_code_snippets_present) == 2) {
						...
						if (sizeof($new_code_snippets_missing) == 2) {
	 ...
	 * @return  none
	 */
	function _checkOldFilesDirsRemaining()
	{
		// Main variable holds the list of files to check 
		$old_files_dirs = array();
		
		$old_files_dirs[] = array(
			'file' => DIR_WS_CLASSES . 'class.CeonString.php',
			);
	...
	function _buildMainPanel()
	{
		...
		$num_config_files_error_messages = sizeof($this->_config_files_error_messages);
		
		$num_modified_core_files_error_messages =
			sizeof($this->_modified_core_files_error_messages);
		
		$num_old_files_dirs_error_messages = sizeof($this->_old_files_dirs_error_messages);
		...
		if ($num_config_files_error_messages > 0) {
			...
			foreach ($this->_config_files_error_messages as $error_message) {
				...
				if (sizeof($error_message['instructions']) > 0) {
	 ...
	 */
	function _buildExampleRewriteRulePanelContent()
	{
		$content = '';
...
SELECT_AND_COPY_JS;
	 ...
	 * @return  none
	 */
	function _buildBasicRewriteRule()
	{
...
## END CEON URI MAPPING REWRITE RULE';
	 ...
	 * @return  none
	 */
	function _buildRewriteRuleWithGuestimatedExclusions()
	{
