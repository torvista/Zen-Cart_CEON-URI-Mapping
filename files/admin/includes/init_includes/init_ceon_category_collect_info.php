 ...
 * $Id: init_ceon_category_collect_info.php xxxx 2019-01-02 20:31:10Z $
 */
...
if (defined('FILENAME_CATEGORY_PRODUCT_LISTING') && $_SERVER['SCRIPT_NAME'] == DIR_WS_ADMIN . (!strstr(FILENAME_CATEGORY_PRODUCT_LISTING, '.php') ? FILENAME_CATEGORY_PRODUCT_LISTING . '.php' : FILENAME_CATEGORY_PRODUCT_LISTING) && (isset($_SESSION['ceon_uri_mapping_insert_category']) || isset($_SESSION['ceon_uri_mapping_update_category']))) {
		$categories_id = isset($_GET['cID']) ? $_GET['cID'] : 0;
		$current_category_id = !empty($_SESSION['ceon_uri_mapping_insert_category']) ? $_SESSION['ceon_uri_mapping_insert_category']['current_category_id'] : $_SESSION['ceon_uri_mapping_update_category']['current_category_id'];
...
if (defined('FILENAME_CATEGORY_PRODUCT_LISTING') && $_SERVER['SCRIPT_NAME'] == DIR_WS_ADMIN . (!strstr(FILENAME_CATEGORY_PRODUCT_LISTING, '.php') ? FILENAME_CATEGORY_PRODUCT_LISTING . '.php' : FILENAME_CATEGORY_PRODUCT_LISTING) && isset($_SESSION['ceon_uri_mapping_move_category_confirm'])) {
		$new_parent_id = isset($_GET['cPath']) ? $_GET['cPath'] : 0;
...
//		next redirect.
if (defined('FILENAME_CATEGORY_PRODUCT_LISTING') && $_SERVER['SCRIPT_NAME'] == DIR_WS_ADMIN . (!strstr(FILENAME_CATEGORY_PRODUCT_LISTING, '.php') ? FILENAME_CATEGORY_PRODUCT_LISTING . '.php' : FILENAME_CATEGORY_PRODUCT_LISTING) && isset($_GET['action']) && $_GET['action'] == 'move_category_confirm') {
		unset($_SESSION['ceon_uri_mapping_move_category_confirm']);
