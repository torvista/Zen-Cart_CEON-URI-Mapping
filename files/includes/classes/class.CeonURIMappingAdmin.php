 ...
 * @version     $Id: class.CeonURIMappingAdmin.php 08 Jan 2026 torvista
 */
...
class CeonURIMappingAdmin extends CeonURIMappingDBLookup
{
	...
	/**
	 * The site's setting for the whitespace replacement character.
	 * TODO: used as string, review type
	 * @var     int
	 * @access  protected
	 */
	 ...
	 * processing time.
	 * TODO: used as a string, review type
	 * @var     array|null
	 * @access  protected
	 */
	 ...
	 *
	 * @param  bool  $load_config Whether the autogeneration configuration should be loaded when instantiating the
	 * class.
	 * @access  public
	 */
	 ...
	 * @return  void
	 */
	protected function _loadAutogenerationSettings(): void
    {
		global $db;
		...
		if (!$load_autogen_settings_result->EOF) {
			$this->_autogen_new = $load_autogen_settings_result->fields['autogen_new'] == 1;
	 ...
	 * @param  int|null  $language_id   The Zen Cart language ID for the mapping.
	 * @param  string|null  $main_page   The Zen Cart page type the mapping is for.
	 * @param  string|null  $query_string_parameters   The query string parameters which are to be passed on to Zen
	 *                                               Cart as if they were in the original URI. ZC often uses this
	 *                                               information to "define" a page type or a particular instance
	 *                                               of a page type.
	 * @param  int|null  $associated_db_id   The ID of a database record to be passed on to ZC. ZC uses this to
	 *                                        identify a particular instance of a page type.
	 * @param  string|null  $alternate_uri   An alternative URI to redirect to, instead of mapping to a Zen Cart
	 *                                     page.
	 * @param  int|null  $redirection_type_code   The redirection type code to be used when redirecting to an
	 *                                             alternate URI.
	 * @param  bool  $avoid_duplicates   Whether to prevent the creation of a duplicate mapping. Should
	 *                                        only ever be disabled if checks have already been made and should
	 *                                        therefore be skipped here for efficiency.
	 * @return  int   A positive value if the mapping was added, a negative integer (constant value) if a
	 *                    mapping already exists for the specified URI or an error was encountered.
	 */
	public function addURIMapping(
        string $uri, ?int $language_id = null, ?string $main_page = null, ?string $query_string_parameters = null,
		?int $associated_db_id = null, ?string $alternate_uri = null, ?int $redirection_type_code = null, bool $avoid_duplicates = true): int
    {
		global $db;
		...
		if (is_null($associated_db_id) || $associated_db_id <= 0) {
			$associated_db_id_quoted = 'NULL';
		} else {
			$associated_db_id_quoted = "'" . $associated_db_id . "'";
		}
		...
		if (is_null($redirection_type_code) || $redirection_type_code <= 0) {
			$redirection_type_code_quoted = 'NULL';
		} else {
			$redirection_type_code_quoted = "'" . $redirection_type_code . "'";
		}
	 ...
	 * @param  int  $language_id   The Zen Cart language ID for the mapping.
	 * @param  bool  $avoid_duplicates   Whether to prevent the creation of duplicate historical
	 ...
	 * @return  int   A positive value if the mapping was made historical, a negative integer (constant value)
	 *                    if an error was encountered.
	 */
	public function makeURIMappingHistorical(string $uri, int $language_id, bool $avoid_duplicates = true): int
    {
		global $db;
		...
		if (is_null($language_id) || $language_id <= 0) {
			// Cannot be null!
			return CEON_URI_MAPPING_MAKE_MAPPING_HISTORICAL_ERROR_DATA_ERROR;
		} else {
			$language_id_quoted = "'" . $language_id . "'";
		}
	 ...
	 * @return  void
	 */
	public function deleteURIMappings(array $selections): void
    {
		global $db;
		...
		foreach ($selections as $column_name => $column_value) {
			if (is_array($column_value)) {
				...
				for ($column_value_i = 0; $column_value_i < $num_column_values; $column_value_i++) {
					...
					} elseif (strtolower((string)$value) === 'not null') {
     ...
     *
     * @access  protected
     * @return bool|null Whether auto-generation of URIs is enabled or disabled.
     */
	protected function _autogenEnabled(): ?bool
    {
	...
	/**
	 * Checks whether the site's settings dictate to prepend the language code to the URI.
	 * TODO: _language_code_add is int|null, so return is not bool
	 * @access  protected
	 * @return  int|null   Whether a unique URI Mapping should be autogenerated by appending an integer to a
	 *                    clashing mapping.
	 */
	protected function _autoLanguageCodeAdd(): ?int
    {
	...
	/**
	 * Checks whether the site's settings dictate that when a mapping clashes, an attempt should be made to
	 * autogenerate a new, unique URI mapping by appending an integer to the clashing mapping.
	 *
	 * @access  protected
	 * @return  bool   Whether a unique URI Mapping should be autogenerated by appending an integer to a
	 *                    clashing mapping.
	 */
	protected function _mappingClashAutoAppendInteger(): bool
    {
	 ...
	 * @param  bool  $strip_slashes   Whether to strip forward slashes from the string.
	 * @return  string    The string as converted for use in a URI.
	 */
	protected function _convertStringForURI(string $text, string $language_code, bool $strip_slashes = true): string
    {
		//global $db;
		...
		if (!is_array($this->_remove_words)) {
            ...
            //TODO: _char_str_replacements is supposedly an array, review type
        ...
        return preg_replace('/\s/', $whitespace_replacement_char, $text);
	}
	 ...
	 */
	protected function _cleanUpURIMapping(string $uri): string
    {
		// Remove any starting or trailing whitespace
		$uri = trim($uri);
