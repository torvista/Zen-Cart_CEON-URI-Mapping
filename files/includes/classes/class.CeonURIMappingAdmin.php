<?php
//steve references to class String changed to (CeonString for php7 as reserved), construct
/**
...
class CeonURIMappingAdmin extends CeonURIMappingDBLookup
{
	 ...
	 * @return  none
	 */
	function _loadAutogenerationSettings()
	{
		global $db;
	 ...
	 */
	function addURIMapping($uri, $language_id = null, $main_page = null, $query_string_parameters = null,
	 ...
	 */
	function makeURIMappingHistorical($uri, $language_id, $avoid_duplicates = true)
	{
		global $db;
	 ...
	 * @return  none
	 */
	function deleteURIMappings($selections)
	{
		global $db;
	 ...
	 *                    clashing mapping.
	 */
	function _mappingClashAutoAppendInteger()
	{
	 ...
	 */
	function _convertStringForURI($text, $language_code, $strip_slashes = true)
	{
		global $db;
		
		/**
		 * Load in the String class to handle multibyte characters and transliteration.
		 */
		...
		if (!is_array($this->_remove_words)) {
			...
			if (sizeof($this->_remove_words) == 1 && (is_null($this->_remove_words[0]) ||
			...
			for ($i = 0, $n = sizeof($char_str_replacement_pairs); $i < $n; $i++) {
				$current_char_str_replacement = explode('=>', $char_str_replacement_pairs[$i]);
				
				if (sizeof($current_char_str_replacement) == 2) {
		...
		if (sizeof($this->_remove_words) > 0) {
		...
		if (sizeof($this->_char_str_replacements) > 0) {
	 ...
	 */
	function _cleanUpURIMapping($uri)
	{
		// Remove any starting or trailing whitespace
		$uri = trim($uri);
