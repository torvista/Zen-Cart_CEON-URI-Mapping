 ...
 * @version     $Id: class.CeonURIMappingHREFLinkBuilder.php 18 Jan 2026 torvista
 */
...
class CeonURIMappingHREFLinkBuilder extends CeonURIMappingDBLookup
{
	...
	public function buildHREFLink(string $base_link, string $main_page, string $parameters, string $connection, bool $add_session_id): bool
    {
		...
		if (preg_match('/.*index.php\?main_page=([^&]+)/i', $main_page, $matches)) {
			...
			$main_page = $matches[1];
			$page = ''; // @TODO determine if $page should equal $main_page
		...
		} elseif (in_array($main_page, $ceon_uri_mapping_product_pages) ||
